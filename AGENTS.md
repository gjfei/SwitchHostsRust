...
**依赖方向：** `app/egui` → `service` → `core`。GUI 不反向被 core 依赖。
...
cargo package-macos                                  # → dist/*.dmg
cargo package-windows                                # → dist/*-setup.exe（Windows）

# 发布（push tag 触发 .github/workflows/release.yml）
#   v0.2.0           — 全部 app/
#   egui-app-v0.2.0  — 仅 egui-app
cargo package-dmg                                    # 同 package-macos
```
...
| 组件 | 路径 |
|------|------|
| GUI main | `app/egui/src/main.rs` |
| 应用状态与帧循环 | `app/egui/src/app.rs`（`SwitchHostsApp`） |
| 面板 | `app/egui/src/panels/` |
| 主题 / 间距 | `app/egui/src/theme.rs` |
| 图标 / 字体资源 | `crates/ui-assets/`、`app/egui/src/icons.rs` |
...
|-------------|---------------------|
| `app/egui/src/panels/` | `src/renderer/components/` |
...
| macOS 标题栏 / traffic lights | `app/egui/src/macos.rs`、`panels/top_bar.rs` |
| macOS bundle | `Packager.toml`（cargo-packager），Bundle ID `app.switchhostsrust` |
...
| GUI smoke | `app/egui/tests/smoke.rs` |
| 共享 fixture | `tests/fixtures/` |

- 集成测试使用 `MockElevation`，不调用真实 sudo
- Core 禁用 doctest（`Cargo.toml`）
- CI：`.github/workflows/build.yml`（先测再三平台 compile）、`release.yml`（tag → macOS .dmg + Windows NSIS）
...
**导入导出：** `crates/core/src/import_export/mod.rs` + `app/egui/src/data_transfer.rs`；导入兼容 SwitchHosts v3/v4/v5 备份 JSON，导出为 v5 格式。

**HTTP API：** 端口 50761，与原版兼容；运行时封装在 `app/egui/src/http_api_runtime.rs`。
